...
class Manager extends Controller
{
    public function show(Request $request)
    {
        if($request->session()->get('user_id')==""){
            ...
            $managerArr = DB::table('manager')->where('delete_status', 0)->get();
            //busArr is the array to store all the data from client table
            return view('manager',compact('managerArr','capsule'));
        }
    }
    ...
    public function create(Request $request)
    {
        if($request->session()->get('user_id')==""){
            ...
            $username = $request->session()->get('user_name');
            $capsule = array('username' => $username); 
            return view('create_manager',compact('capsule'));   
        }
    }
        ...
        if($request->session()->get('user_id')==""){
            ...
            if ($query) {
                if (DB::table('admin')->where('username', '=',$request->session()->get('user_name'))->exists()) {
                    ...
                    $arr = DB::table('manager')->where('username', '=',$request->input('username'))->get('id');
    ...
    public function edit($id,Request $request)
    {
        if($request->session()->get('user_id')==""){
            ...
            $username = $request->session()->get('user_name');
            $capsule = array('username' => $username);
            $managerArr = DB::table('manager')->where('id',$id)->get();
            // echo $clientArr;
            return view('manager_edit',compact('managerArr','capsule'));   
        }
    }
    ...
    public function update(Request $request, $id)
    {
        if($request->session()->get('user_id')==""){
            ...
            $detail =  DB::table('manager')->where('id', $id)->get();
